	...
	def get(self, request, id=None):
		...
		for blog_rec in blog_name:
			...
			for comment_data in comment_obj :
				...
				user_data= CustomUser.objects.filter(id=comment_data['user']).values('email')
				user_record = []
...
class addblogViews(TemplateView):
	...
	def post(self, request, id=None):
		...
		if len(blog)>0:
			...
			blog_comment = Comment.objects.create(text=blog_comment,content_object=blog_ob,user=CustomUser.objects.get(id=user_name) )
		return render(request, self.template_name, context_data)
...
class editBlogComemnt(TemplateView):
	...
	def post(self, request, id=None):
		...
		if len(blog_comments)>0:
			#blog_ob = blog_model.objects.create(blog_heading=blog,created_by=CustomUser.objects.get(id=user_name))
			blog_commnt = Comment.objects.create(text=blog_comments,content_object=blog_model.objects.get(id=id),user=CustomUser.objects.get(id=user_name) )
